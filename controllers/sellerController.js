  ...
  try {
    upload(req, res, async (err) => {
      ...
      if (updateData.profileImage) {
        ...
        const img = req.files?.find(el => el.originalname === updateData.profileImage);
        if(img){
          const fileBuffer = img.buffer;
          const fileOriginalName = img.originalname;
          ...
          updateobj.profilePicture = imgURL
        }

      }
      if (updateData.coverImage) {
        const img = req.files?.find(el => el.originalname === updateData.coverImage);
        if(img){
          const fileBuffer = img.buffer;
          const fileOriginalName = img.originalname;
          // const imageFile = new File(fileBuffer,fileOriginalName)
